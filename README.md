...
- **ヒートマップ**：hour bucket平均をJSTの曜日と`00-02`から`22-24`までの2時間帯へ分類し、該当bucketを等価平均
...
ヒートマップの色は期間内の相対値ではなく、`0–2未満`、`2–4未満`、`4–6未満`、`6–8未満`、`8室以上`の固定scaleです。各セルには平均値と元になったhour bucket数`n`も表示します。

観測がないhour bucketは0室やnot_availableとして補完せず、未観測として統計から除外します。30日分がまだ蓄積されていない場合も存在するデータだけで計算し、観測日数、観測hour数、coverageを併記します。
